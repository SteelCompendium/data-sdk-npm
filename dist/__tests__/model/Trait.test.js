    ...
    inputFiles.forEach(inputFile => {
        ...
        if (fileExtension === '.yaml' || fileExtension === '.yml') {
            test(`${baseName}.yaml to JSON`, () => {
                ...
                const trait = Trait_1.Trait.read(new yaml_1.YamlReader(Trait_1.Trait.modelDTOAdapter), inputYaml);
                const outputJson = new json_1.JsonWriter().write(trait);
        ...
        if (fileExtension === '.json') {
            test(`${baseName}.json to YAML`, () => {
                ...
                const trait = Trait_1.Trait.read(new json_2.JsonReader(Trait_1.Trait.modelDTOAdapter), inputJson);
                const outputYaml = new yaml_2.YamlWriter().write(trait);
