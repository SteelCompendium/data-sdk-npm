...
exports.PrereleasePdfStatblockReader = void 0;
...
class PrereleasePdfStatblockReader {
    read(text) {
        ...
        statblock.characteristics = new model_1.Characteristics(0, 0, 0, 0, 0);
        ...
        const isNewToken = (line) => {
            ...
            if (/^(.+?)\s+\(\s*(Main Action|Action|Maneuver|Free Triggered Action|Triggered Action|Triggered|Villain Action\s*\d+)\s*\)/.test(line))
                return true;
        ...
        const pushCurrent = () => {
            ...
            ability.effects = new Effects_1.Effects(current.effects || []);
            statblock.abilities.push(new Ability_1.Ability(ability));
            current = null;
        };
        while (idx < lines.length) {
                ...
                if (outcomeParts) {
                    ...
                    if (lookahead >= lines.length || !/^([✦★✸])\s*/.test(lines[lookahead])) {
                        if (!current.roll && current.effects.length > 0) {
                            ...
                            if (lastEffect instanceof model_1.MundaneEffect && lastEffect.effect.includes("test")) {
                                ...
                                current.effects[current.effects.length - 1] = new model_1.PowerRollEffect(rollEffect);
                                current.outcomes = [];
                            }
                        }
                            ...
                            current.effects.push(new model_1.PowerRollEffect(rollEffect));
                            current.outcomes = [];
                        }
                    }
                    idx = lookahead;
            ...
            if (ef && current) {
                ...
                current.effects.push(new model_1.MundaneEffect({ effect: effectText }));
            ...
            if (!current && isNewToken(line)) {
                ...
                const flushUnnamedEffect = () => {
                    if (currentUnnamedEffectLines.length > 0) {
                        effects.push(new model_1.MundaneEffect({ effect: currentUnnamedEffectLines.join(" ") }));
                ...
                while (lookahead < lines.length && !isNewToken(lines[lookahead])) {
                    ...
                    if (titleCaseWords > 1 && titleCaseWords < words.length) {
                        ...
                        effects.push(new model_1.MundaneEffect({ effect: description, name: effectName }));
                    }
                    else {
                ...
                flushUnnamedEffect();
                statblock.traits.push(new Trait_1.Trait({ name: traitName, effects: new Effects_1.Effects(effects) }));
                idx = lookahead;
                continue;
            }
            if (!line.trim()) {
...
//# sourceMappingURL=PrereleasePdfStatblockReader.js.map