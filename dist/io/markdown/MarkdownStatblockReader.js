...
class MarkdownStatblockReader {
    ...
    read(content) {
        ...
        while (i < mainLines.length) {
            if (/^\s*>\s?/.test(mainLines[i])) {
                ...
                traits.push(new model_1.Trait({ name: traitAbility.name, effects: traitAbility.effects }));
            }
            else {
                i++;
            }
    ...
    // ──────────────────────────────────────────────────────────────────────────
    parseStatblockTable(lines, partial) {
        if (!lines.length)
            return;
        const isSep = (s) => {
        ...
        if (mLevel)
            partial.level = parseInt(mLevel[1], 10) || 0;
        ...
        for (let r = 2; r < rows.length; r++) {
            ...
            for (let c = 0; c < 5; c++) {
                ...
                const setList = (val) => val === '-' ? [] : val.split(',').map(s => s.trim()).filter(Boolean);
                switch (label) {
                    case 'size':
                        if (valueRaw)
                            partial.size = valueRaw;
                        break;
                    case 'speed':
                        if (valueRaw && valueRaw !== '-')
                            partial.speed = parseInt(valueRaw, 10) || 0;
                        break;
                    case 'stamina':
                        if (valueRaw)
                            partial.stamina = valueRaw;
                        break;
                    case 'stability':
                        if (valueRaw && valueRaw !== '-')
                            partial.stability = parseInt(valueRaw, 10) || 0;
                        break;
                    case 'free strike':
                        if (valueRaw && valueRaw !== '-')
                            partial.freeStrike = parseInt(valueRaw, 10) || 0;
                        break;
                    case 'immunities':
